...
| `sync.ts` | Declarative reconcile of shared-path links across all slots via the manifest; prunes removed links; GCs manifest entries for vanished slots; self-heals + applies the detach registry per slot. Idempotent. `--global` reconciles HOME-scope links instead (`~/<tail>` → `~/.worm/shared/<tail>`). |

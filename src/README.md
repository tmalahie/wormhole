...
| `hook.ts` | `worm hook trigger <event>` — internal recipe-hook dispatcher invoked by each slot's `settings.local.json` (one static entry per event). Events: `pre-tool-use` (filter), `user-prompt-submit` (context), `session-start` / `session-end` / `stop` (run). Resolves the live slot, runs enabled **project-scope** recipes with injected env, and owns logging. `--global` runs **global-scope** recipes (`autosync`) from `~/.worm/config.json` with NO project context (the form `worm sync --global` writes into `~/.claude/settings.json`). Must never throw; fails open on the hot path. Recipe code lives in `src/recipes/<name>/`. |

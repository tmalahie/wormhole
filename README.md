...
- **`recipes`** — composable capabilities, keyed by name (provider-style). A recipe is **enabled iff its key is present**; each value is validated by that recipe's own schema. Two kinds of thing back a recipe, kept deliberately separate:
  ...
  - **`notifyPendingInput`** (`{}`, **global**) — fires an OS notification when Claude is waiting on you (your input is pending): *"Response ready"* on `Stop`, *"Waiting for approval"* on `PermissionRequest`. On macOS it uses `terminal-notifier` (clicking focuses the VS Code window for that project) and falls back to `osascript`; on Linux, `notify-send`. Debounces the burst of intermediate `Stop` yields a background-agent turn produces (one notification on the final answer) and ignores sub-agent events. Shares the notification backend (`src/recipes/_lib/notify.js`) with `autosync`.

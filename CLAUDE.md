...
- Hooks receive `WORM_PROJECT_ROOT`, `WORM_SLOT`, `WORM_SLOT_INDEX` (0-based numeric), and `WORM_BRANCH` env vars. Build the env with `hookEnv(projectRoot, slot, branch)` from `core/hooks.ts` — don't inline the object at the call site, and don't pass `cwd` as a bare string to `runHook`.

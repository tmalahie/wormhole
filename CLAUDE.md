...
- **Recipe scope.** A recipe is `scope: "project"` (default) or `"global"` (`Recipe.scope`). Project recipes are wired per-slot into `settings.local.json` by `worm sync` / `universe add`, and dispatched by `worm hook trigger <event>` (resolves the slot from cwd). The one global recipe, **`autosync`**, is declared in the **global** `~/.worm/config.json` `recipes` block, wired by `worm sync --global` (`applyGlobalRecipeWiring`) into `~/.claude/settings.json`, and dispatched by `worm hook trigger --global <event>` (`runGlobalRecipeHooks`) with **no project context** — so it fires for every Claude session machine-wide. `enabledRecipes(recipes, scope)` filters by scope, so a global recipe is never wired into a slot and vice-versa. autosync's script (`src/recipes/autosync/sync-worm-home.js`) acts on `~/.worm` via `WORM_HOME`, never auto-resolves a conflict (clean `rebase --abort` + a `.autosync-conflict.json` marker surfaced by `worm status` + OS notification); `ensureGlobalRoot` gitignores machine-local state (manifests, logs, the marker) so it isn't pushed across machines.
